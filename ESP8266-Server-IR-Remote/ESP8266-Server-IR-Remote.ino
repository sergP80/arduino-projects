...
String generateButton(String colSize, String id, String text, String bgColor, String url) {

  return  "<div class=\"" + colSize + "\" style=\"text-align: center\">\n" +
          "    <button id=\"" + id + "\" type=\"button\" class=\"btn btn-default\" style=\"width: 100%; background-color: " + bgColor + ";\" onclick='makeAjaxCall(\"" + url + "\")'>" + text + "</button>\n" +
          "</div>\n";
}

String generateStateButton(String colSize, String id, String text, String url) {

  return  "<div class=\"" + colSize + "\" style=\"text-align: center\">\n" +
          "    <button id=\"" + id + "\" type=\"button\" class=\"btn btn-default\" style=\"width: 100%;\" state-attr=\"false\" onclick='makeStateCall(\"" + url + "\")'>" + text + "</button>\n" +
          "</div>\n";
}
...
void connectToAP()
{
  ...
  wifiManager.setSaveConfigCallback(saveConfigWifiCallBack);  
...
void handleRoot() {
  ...
  website = website + generateStateButton("col-xs-4", "tvpower", "TV Power",   "tvpower");
  website = website + generateStateButton("col-xs-4", "sspower", "SS Power",   "sspower");
  website = website + generateStateButton("col-xs-4", "satpower", "Sat Power", "satpower");
  ...
  // —---------------------— Volume Controls —------------------------
  website = website + rowDiv;
  website = website + generateButton("col-xs-12", "up", "Vol Up", "#D4AC0D", "up");
  website = website + endDiv;
  website = website + rowDiv;
  website = website + generateButton("col-xs-12", "down", "Vol Down", "#D35400", "down");
  website = website + endDiv;
  // —---------------------— Satelite Controls —------------------------
  // Not working well
  //  website = website + rowDiv;
  ...
  // —---------------------— Change WiFi —------------------------
  website = website + rowDiv;
  website = website + generateButton("col-xs-12", "wifi-change-id", "Change WiFi connect", "#A32DE5", "wifi-change");
  website = website + endDiv;
  ...
  website = website + "    <script> function makeAjaxCall(url){$.ajax({\"url\": url})};\n";
  website = website + "    var stateOnColor='#16A085';\n var stateOffColor='#EC7063';\n function makeStateCall(url){ var currentState = $(this.id).attr('state-attr');\n";
  website = website +      "$(this.id).attr('state-attr', !currentState); \n";
  website = website +      "$(this.id).css('background-color', !currentState ? stateOnColor : stateOffColor);\n";
  website = website +      "$.ajax({\"url\": url})\n};</script>\n";
  website = website + "  </body>\n";
  website = website + "</html>\n";
...
{
  Serial.println("***** New wifi settings were saved *****");
  ...
  Serial.println("Web server started success");
}
...
void loop(void) {
  if (server)
  {
    ...
    Serial.println("Fail to init IR web-server. Try again...");
    ESP.reset();
    connectToAP();   
  }
